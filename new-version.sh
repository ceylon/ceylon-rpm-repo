...
perl -pi -e "s/^(%changelog)$/\$1\n$CHDATE $CHAUTHOR ${VERSION}-0/" dist-pkg/ceylon.spec
perl -pi -e "s/^(%major_version) .*$/\$1 ${VS[0]}/" dist-pkg/ceylon.spec
perl -pi -e "s/^(%minor_version) .*$/\$1 ${VS[1]}/" dist-pkg/ceylon.spec
perl -pi -e "s/^(%micro_version) .*$/\$1 ${VS[2]}/" dist-pkg/ceylon.spec
perl -pi -e "s/^(%alternatives_version) .*$/\$1 ${PRIO}/" dist-pkg/ceylon.spec
